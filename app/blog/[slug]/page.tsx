...
// Updated type to reflect lean() output
interface PopulatedPost {
  _id: string;
  title: string;
  content: string;
...
async function getPost(slug: string): Promise<PopulatedPost | null> {
  try {
    ...
    if (!post) {
      console.log(`No published post found for slug: ${slug}`); // Debug log
      return null;
    }
    return JSON.parse(JSON.stringify(post)); // Ensures plain object
  } catch (error) {
    console.error("Database error fetching post:", error);
    return null;
  }
